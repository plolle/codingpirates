<#
  Henter de nye skaermbilleder ind i lektion 07-advanced-you-lose.
...
$map = @(
  @{ Id = "-8";  Name = "01-lose-text.png";       Mask = @() }
  @{ Id = "-10"; Name = "02-you-lose-scene.png";  Mask = @() }
  @{ Id = "-11"; Name = "03-change-scene.png";    Mask = @() }
  @{ Id = "-12"; Name = "04-try-again-event.png"; Mask = @() }
  @{ Id = "-14"; Name = "05-y-position.png";      Mask = @() }
  @{ Id = "-15"; Name = "06-death-events.png";    Mask = @() }
)
...
foreach ($m in $map) {
  ...
  "{0,-28} <- {1}  ({2} x {3}){4}" -f $m.Name, $file.Name, $bmp.Width, $bmp.Height, $(if($masked){"  [$masked omraade(r) maskeret]"}else{""})
  $bmp.Dispose()
}
